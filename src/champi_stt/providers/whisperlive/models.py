...
from champi_stt.providers.whisperlive.enums import DeviceType, ComputeType, ModelSize, LoggingStrings
from champi_stt.providers.whisperlive.events import STTSignalManager
from champi_stt.providers.whisperlive.enums import ModelEvents, LifecycleEvents
        ...
        if device is None or device == DeviceType.AUTO.value:
            ...
            if use_cuda and torch.cuda.is_available():
                try:
                    ...
                    logger.warning(
                        LoggingStrings.CUDA_FALLBACK_TO_CPU.value.format(e)
                    )
        ...
        if compute_type is None or compute_type == ComputeType.AUTO.value:
            if device == DeviceType.CUDA.value:
                try:
                    ...
                    compute_type = ComputeType.FLOAT16.value if major >= 7 else ComputeType.FLOAT32.value
                except Exception:
...
class ModelCacheManager:
    ...
    async def save_cache_metadata(self, config: WhisperLiveConfig, cache_key: str) -> None:
    ...
    async def get_cached_model(self, cache_key: str) -> Optional[WhisperModel]:
...
class ModelManager:
    ...
    async def initialize(self) -> WhisperModel:
        ...
        try:
            ...
            device, compute_type = DeviceManager.auto_detect_device_settings(self.config)
            self.config.device = device
            self.config.compute_type = compute_type

            logger.debug(LoggingStrings.DEVICE_AUTO_DETECTED.value.format(device, compute_type))
            ...
            if not self.cache_manager.is_model_cached(self._model_key):
                await self.cache_manager.save_cache_metadata(self.config, self._model_key)
    ...
    async def get_model_info(self) -> Dict[str, Any]:
        ...
        info = {"status": "loaded", "model_size": self.config.model_size, "device": self.config.device,
                "compute_type": self.config.compute_type, "cache_key": self._model_key,
                "is_multilingual": self._model.model.is_multilingual,
                "supported_languages": list(self._model.supported_languages) if hasattr(self._model,
                                                                                        'supported_languages') else [],
